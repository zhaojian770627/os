	...
	core_start_sector equ 0x00000001 ;常熟，内核的起始逻辑扇区号
...
SECTION mbr vstart=0x00007c00
	...
	;; 创建2#描述符，保护模式下的代码段描述符
	mov	dword[ebx+0x10],0x0000ffff ;基地址为0，界限0xfffff,DPL=00
	mov	dword[ebx+0x14],0x00cf9200 ;4KB粒度，代码段描述符，向上扩展
	;; 初始化描述符表寄存器GDTR
	...
	;; 以下进行保护模式......
	jmp	dword 0x0008:flush ;16位的描述符选择子:32位偏移,清流水线并串行化处理器
	[bits 32]
flush:
...
pge:
	...
	mov	[ebx+0x000],edx	;写入目录项(页表的物理地址和属性),此目录仅用于过度
...
.b1:
	...
	inc	esi
	cmp	esi,256		;仅低端1MB内存对应的页才有效的
	jl	.b1

	;; 令CR3寄存器指向页目录，并正式开启页目录
